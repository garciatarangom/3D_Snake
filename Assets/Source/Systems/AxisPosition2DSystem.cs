...
namespace Source.Systems
{
    ...
    public class AxisPosition2DSystem: MonoBehaviour
    {
        ...
        [SerializeField] private Vector2DValue direccion = null;
        ...
        private void Update()
        {
            if (Input.GetAxis("Horizontal")!= 0) direccion.value = new Vector2(Input.GetAxis("Horizontal"), 0);
            if (Input.GetAxis("Vertical")!= 0) direccion.value = new Vector2(0, Input.GetAxis("Vertical"));
            axis.X = direccion.X * speed * Time.deltaTime * (1 - dampening);
            axis.Y = direccion.Y * speed * Time.deltaTime * (1 - dampening);
        }
    }
}
